...
const brochuresQuery = `*[_type == "projects"][0]{
  projectEntries[]{
    _key,
    name,
    location,
...
type ProjectEntry = {
  ...
  brochures: { _key: string; url: string; originalFilename?: string }[] | null;
...
export default function BrochuresPage() {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const data = await client.fetch<{ projectEntries: ProjectEntry[] }>(brochuresQuery)
  ...
  const handleDeleteBrochure = async (projectKey: string, brochureKey: string) => {
    if (!confirm('Are you sure you want to delete this brochure?')) return

    setDeletingKey(brochureKey)
    try {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-[#0f1d33]">Brochures</h1>
        <p className="mt-2 text-sm text-[#5a6a82]">
          Manage and upload project brochures (PDFs) to Sanity.
        </p>
      </div>
            ...
            <table className="w-full text-sm text-left relative">
              <thead className="bg-[#f7f8fa] text-[#5a6a82] sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-6 py-4 font-medium">Current Brochures</th>
                      ...
                      {project.brochures && project.brochures.length > 0 ? (
                        <div className="flex flex-col gap-2">
                          {project.brochures.map((brochure, index) => (
                            <div key={brochure._key} className="flex items-center justify-between gap-2 group w-full max-w-[300px]">
                              <a 
                                href={brochure.url} 
                                target="_blank" 
                        ...
                        <span className="text-[#5a6a82] italic text-sm">No brochures uploaded</span>
                      )}
                    </td>
