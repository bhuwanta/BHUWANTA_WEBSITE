...
import { Plus, Edit2, Trash2, X, Search, Globe, FilterX, Download, MessageCircle, Megaphone, Settings, ArrowUp, ArrowDown, ArrowUpDown, Check, RefreshCw, Mail } from 'lucide-react'
...
export default function LeadsClient({ initialLeads, userRole = 'Admin' }: { initialLeads: any[], userRole?: string }) {
  ...
  const [selectedReportType, setSelectedReportType] = useState('evening')
  const [isSendingReport, setIsSendingReport] = useState(false)

  const handleSendReport = async () => {
    try {
      setIsSendingReport(true)
      const res = await fetch('/api/crm/send-report', {
        method: 'POST',
      ...
      alert(`Success! ${data.period} sent to info@bhuwanta.com.`)
      ...
      <div className="flex flex-col gap-4 bg-white p-4 rounded-xl border border-[#e8ecf2] shadow-sm">
        ...
        <div className="flex items-center gap-2 ml-auto mt-2 sm:mt-0">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 justify-center rounded-full bg-[#1e3a5f] px-4 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-[#0f1d33] transition-colors"
          >
      ...
      {isEmailModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-[#e8ecf2] bg-[#f7f8fa]">
              <h3 className="font-semibold text-[#0f1d33] flex items-center gap-2">
                <Mail className="w-4 h-4" /> Send Email Report
              </h3>
              <button onClick={() => setIsEmailModalOpen(false)} className="text-[#5a6a82] hover:text-[#0f1d33] transition-colors p-1 rounded-full hover:bg-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 flex flex-col gap-4">
              <p className="text-sm text-[#5a6a82]">
                Select the time block you want to generate a report for. This will instantly send the PDFs and Excel files to <strong>info@bhuwanta.com</strong>.
              </p>
              
              <div className="flex flex-col gap-3 mt-2">
                <label className="flex items-start gap-3 p-3 rounded-lg border border-[#e8ecf2] hover:border-[#1e3a5f] cursor-pointer transition-colors bg-[#f7f8fa]">
                  <input type="radio" name="reportType" value="morning" checked={selectedReportType === 'morning'} onChange={(e) => setSelectedReportType(e.target.value)} className="mt-1" />
                  <div>
                    <div className="font-medium text-[#0f1d33] text-sm">Morning Report</div>
                    <div className="text-xs text-[#5a6a82]">6 PM Yesterday - 9 AM Today</div>
                  </div>
                </label>
                
                <label className="flex items-start gap-3 p-3 rounded-lg border border-[#e8ecf2] hover:border-[#1e3a5f] cursor-pointer transition-colors bg-[#f7f8fa]">
                  <input type="radio" name="reportType" value="mid-day" checked={selectedReportType === 'mid-day'} onChange={(e) => setSelectedReportType(e.target.value)} className="mt-1" />
                  <div>
                    <div className="font-medium text-[#0f1d33] text-sm">Mid-Day Report</div>
                    <div className="text-xs text-[#5a6a82]">9 AM Today - 12 PM Today</div>
                  </div>
                </label>
                
                <label className="flex items-start gap-3 p-3 rounded-lg border border-[#e8ecf2] hover:border-[#1e3a5f] cursor-pointer transition-colors bg-[#f7f8fa]">
                  <input type="radio" name="reportType" value="afternoon" checked={selectedReportType === 'afternoon'} onChange={(e) => setSelectedReportType(e.target.value)} className="mt-1" />
                  <div>
                    <div className="font-medium text-[#0f1d33] text-sm">Afternoon Report</div>
                    <div className="text-xs text-[#5a6a82]">12 PM Today - 3 PM Today</div>
                  </div>
                </label>
                
                <label className="flex items-start gap-3 p-3 rounded-lg border border-[#e8ecf2] hover:border-[#1e3a5f] cursor-pointer transition-colors bg-[#f7f8fa]">
                  <input type="radio" name="reportType" value="evening" checked={selectedReportType === 'evening'} onChange={(e) => setSelectedReportType(e.target.value)} className="mt-1" />
                  <div>
                    <div className="font-medium text-[#0f1d33] text-sm">6 PM Evening Report</div>
                    <div className="text-xs text-[#5a6a82]">3 PDFs: 3PM-6PM, Today Total, All Time</div>
                  </div>
                </label>
              </div>
            </div>
            
            <div className="p-4 border-t border-[#e8ecf2] flex justify-end gap-3 bg-white">
              <button
                onClick={() => setIsEmailModalOpen(false)}
                className="px-4 py-2 rounded-lg text-sm font-medium text-[#5a6a82] hover:text-[#0f1d33] hover:bg-[#f3f5f8] transition-colors"
              ...
              <button
                ...
                disabled={isSendingReport}
                className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-[#1e3a5f] text-white text-sm font-medium hover:bg-[#0f1d33] transition-colors disabled:opacity-50"
              >
                {isSendingReport ? (
                ...
                {isSendingReport ? 'Sending...' : 'Send Email'}
              </button>
            </div>
          </div>
        </div>
