...
export async function POST(request: Request) {
  try {
    ...
    if (!['morning', 'mid-day', 'afternoon', 'evening'].includes(reportType)) {
    ...
    if (reportType === 'evening') {
      ...
      if (allTimeLeads) {
        ...
        const threePmIst = new Date(midnightIst.getTime() + (15 * 60 * 60 * 1000)); // 3 PM Today
        ...
        emailHtml = `
          <h2>Bhuwanta 6 PM Leads Report (Manual Trigger)</h2>
      ...
      emailHtml = `
        <h2>Bhuwanta Leads Report (Manual Trigger)</h2>
    ...
    if (attachments.length > 0) {
      ...
      const { error: emailError } = await resend.emails.send({
        ...
        subject: `[Bhuwanta CRM] ${reportPeriod} (Manual)`,
        html: emailHtml + `<br/><p><small>This email was manually triggered from the CRM dashboard.</small></p>`,
        attachments
      });
