...
type SortCol = 'srno' | 'date' | 'project' | 'soldBy' | 'role' | 'percentage' | 'amount' | 'status';
    ...
    <div className="p-4 md:p-6 bg-[#f7f8fa] h-full flex flex-col">
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6 shrink-0">
        ...
        <div className="relative shrink-0">
          ...
          <input
            type="text"
            placeholder="Search project, seller, role..."
            value={searchQuery}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 shrink-0">
        ...
        <StatCard label="Total Accrued" value={formatCurrency(totals.totalEarned)} />
      </div>
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead className="sticky top-0 z-10">
                <tr className="bg-[#f3f5f8] text-[#5a6a82] text-xs uppercase tracking-wider font-semibold border-b border-[#e8ecf2]">
                  <th className="p-4 w-16 cursor-pointer hover:bg-[#e8ecf2] transition-colors select-none" onClick={() => handleSort('srno')}>
                    Sr. No. <SortIcon col="srno" />
                  </th>
                  <th className="p-4 cursor-pointer hover:bg-[#e8ecf2] transition-colors select-none" onClick={() => handleSort('date')}>
                    Date <SortIcon col="date" />
                  </th>
                  <th className="p-4 cursor-pointer hover:bg-[#e8ecf2] transition-colors select-none" onClick={() => handleSort('project')}>
                    Project <SortIcon col="project" />
                  </th>
                  <th className="p-4 cursor-pointer hover:bg-[#e8ecf2] transition-colors select-none" onClick={() => handleSort('soldBy')}>
                  ...
                  <th className="p-4 cursor-pointer hover:bg-[#e8ecf2] transition-colors select-none" onClick={() => handleSort('amount')}>
                    Amount <SortIcon col="amount" />
                  </th>
                  <th className="p-4 cursor-pointer hover:bg-[#e8ecf2] transition-colors select-none" onClick={() => handleSort('status')}>
                ...
                {sorted.map((p, index) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-[#f7f8fa] transition-colors">
                      <td className="p-4 text-sm text-[#5a6a82] whitespace-nowrap">{index + 1}</td>
                      <td className="p-4 text-sm text-[#5a6a82] whitespace-nowrap">{reg?.submitted_at ? new Date(reg.submitted_at).toLocaleDateString() : '—'}</td>
                      <td className="p-4 whitespace-nowrap">
                        <span className="font-semibold text-[#0f1d33]">{reg?.s_projects?.name || '—'}</span>
                        <div className="text-xs text-[#5a6a82]">{reg?.s_areas?.name}</div>
                      </td>
                      <td className="p-4 whitespace-nowrap">
                        <div className="flex items-center gap-1.5">
                          <User className="w-3.5 h-3.5 text-[#5a6a82]" />
                          <span className="text-sm text-[#0f1d33]">{seller?.full_name || '—'}</span>
                        </div>
                      </td>
                      <td className="p-4 whitespace-nowrap">
                        {seller?.role ? (
                      ...
                      <td className="p-4 text-sm text-[#0f1d33] font-medium whitespace-nowrap">
                        {Number(p.commission_percentage)}%
                        {Number(p.previous_tier_percentage) > 0 && (
                          <div className="text-[11px] font-normal text-[#a0abbb]">
                            {Number(p.tier_percentage)}% − {Number(p.previous_tier_percentage)}%
                          </div>
                        )}
                      </td>
                      <td className="p-4 whitespace-nowrap">
                        <span className="font-bold text-[#0f1d33]">{formatCurrency(Number(p.amount))}</span>
                        {wasRounded && <div className="text-[11px] text-[#a0abbb]">rounded up from {formatCurrency(Number(p.computed_amount))}</div>}
                      </td>
                      <td className="p-4 whitespace-nowrap">
