...
import { resetPasswordByBhuwantaIdAction } from './actions';
...
export default function SecurityPage() {
  ...
  // Form state
  const [bhuwantaId, setBhuwantaId] = useState('');
  const [newPassword, setNewPassword] = useState('');
  const [confirmPassword, setConfirmPassword] = useState('');
  const [showPassword, setShowPassword] = useState(false);
  
  // Track resets in the current session to prevent consecutive identical resets
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!bhuwantaId || !newPassword || !confirmPassword) return;
    ...
    const targetId = bhuwantaId.toUpperCase();
    ...
    const confirmed = window.confirm(`CRITICAL WARNING:\n\nAre you absolutely sure you want to reset the password for ID: ${targetId}?\n\nThis will instantly override their current password.`);
    if (!confirmed) return;

    setLoading(true);
    setMessage(null);
    
    const res = await resetPasswordByBhuwantaIdAction(bhuwantaId, newPassword);
      ...
      <div className="bg-white border border-[#e8ecf2] shadow-sm rounded-xl p-6 flex-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="border border-[#e8ecf2] rounded-xl p-5 flex flex-col items-start bg-[#f7f8fa]/50 hover:bg-[#f7f8fa] transition-colors">
            ...
            <p className="text-sm text-[#5a6a82] mb-4">Reset the password for any user using their Bhuwanta ID.</p>
            <button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0f1d33]/40 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleResetPassword} className="p-4 md:p-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-[#0f1d33] mb-1.5">Bhuwanta ID</label>
                <input 
                  type="text" 
                  required
                  value={bhuwantaId}
                  onChange={(e) => setBhuwantaId(e.target.value.toUpperCase())}
                  placeholder="e.g. BHT-..."
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button 
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-[#5a6a82] hover:text-[#0f1d33] focus:outline-none"
                  >
              ...
              <div className="pt-4 border-t border-[#e8ecf2] flex justify-end gap-3 mt-6">
                <button 
                  ...
                  className="px-4 py-2 text-sm font-semibold text-[#5a6a82] hover:bg-[#f3f5f8] rounded-lg transition-colors"
                >
                  Cancel
                </button>
                <button 
